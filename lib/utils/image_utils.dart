...
class ImageUtils {
  ...
  static imageLib.Image convertBGRA8888ToImage(CameraImage cameraImage) {
    imageLib.Image img = imageLib.Image.fromBytes(cameraImage.planes[0].width as int, cameraImage.planes[0].height as int, cameraImage.planes[0].bytes, format: imageLib.Format.bgra);
    return img;
  }
  ...
  static imageLib.Image convertYUV420ToImage(CameraImage cameraImage) {
    ...
    final int uvPixelStride = cameraImage.planes[1].bytesPerPixel as int;
  ...
  static Future<File> saveImage(imageLib.Image image) async {
    List<int> jpeg = imageLib.JpegEncoder().encodeImage(image);
    final appDir = await getTemporaryDirectory();
    final appPath = appDir.path;
    final fileOnDevice = File('$appPath/out${Random().nextInt(10000)}.jpg');
    await fileOnDevice.writeAsBytes(jpeg, flush: true);
    return fileOnDevice;
  }

  static List<List<int>> convertGreyImg(imageLib.Image image) {
    int w = image.width;
    int h = image.height;
    List<int> pixels = List.generate(h * w, (index) => image.getPixel(index % w, index ~/ w));
    List<List<int>> result = List.generate(h, (index) => List.filled(w, 0));
    int alpha = 0xFF << 24;
    for (int i = 0; i < h; i++) {
      for (int j = 0; j < w; j++) {
