...
class _SignupScreenState extends State<SignupScreen> {
    ...
    ScannerUtils.detect(
      ...
      if (results is List<Face>) {
        ...
        if (_listFace.length == 1) {
          ...
          if (qualityScore <= 500)
            warningMsg =
                "Vui lòng đưa lại gần hoặc làm sạch camera hoặc đưa ra khu vực đủ sáng";
          if (qualityScore > 500) {
            onSignup();
          }
          setState(() {
  ...
  void onSignup() async {
    ...
    if (_timer != null) {
      List<Offset> _listPoint =
          _listFace[0].getContour(FaceContourType.noseBottom)!.positionsList;
